...
public class ExcelUtils {
	...
	public static void createExcelFile(String fileName, String sheetName) {
		try {
			...
			excelWSheet = excelWBook.createSheet(fileName);
			excelWBook.write(excelFile);
			excelFile.close();
		} catch (Exception e) {
			System.out.println("Error of Creating ExcelFile.");
		}
	}
	...
	public static String[] getExcelDataInAColumn(String ExcelSheetPath, String sheetname) {
		...
		try {
			...
			for (int i = 0; i < totalNoOfRows-1; i++) {
				...
				arrayExcelData[i] = excelWSheet.getRow(i+1).getCell(0).toString();
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
